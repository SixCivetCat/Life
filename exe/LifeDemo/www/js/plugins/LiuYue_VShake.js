 ...
 *----------------Script Function----------------
 *
 * Zzy.VSF.IsHShake(true/false)       
 * -这会开启/关闭水平震动
 *
 * Zzy.VSF.IsVShake(true/false)       
 * -这会开启/关闭垂直震动
 *
 * Zzy.VSF.HForce(value)                    
 * -修改水平震动比例,这可以是一个小数
 *
 * Zzy.VSF.VForce(value)                         
 * -修改垂直震动比例,这可以是一个小数
 *
 *
 *
 ...
 *
 *
 * 我叫坂本：v1.02 拓展脚本方法,修复垂直震动算法错误问题
 * 我叫坂本：v1.01 兼容旧存档
 * 我叫坂本：v1.00 完成插件功能
 *
 *----------------------------------------------------------
...
Zzy.Param.VSFHForce = Number(Zzy.Parameters['HForce']);
Zzy.Param.VSFVForce = Number(Zzy.Parameters['VForce']);
...
Spriteset_Base.prototype.updatePosition = function() 
{
	...
	{
		this.y += Math.round($gameScreen.ZzyVSFVShake());//震动幅度
	}
};
...
{
	var rateShake = this._shake;
	return rateShake * $gameSystem.GetZzyVSFVForce();
}
